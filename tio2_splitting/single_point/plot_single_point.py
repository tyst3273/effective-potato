...
def plot_single_point(hdf5_file='phonon_angmom_calcs.hdf5',num=1000):
    ...
    for bb in range(nb):
        ...
        ax2.plot(B[:-1], np.abs(np.diff(evals[:,bb]))/0.1, c='k', ms=0.0, marker='o', lw=1)
    ...
    ax2.plot(B[:-1], np.abs(np.diff(evals[:,0]))/0.1, c='r', ms=0.0, marker='o', lw=1)
    ax2.plot(B[:-1], np.abs(np.diff(evals[:,1]))/0.1, c='r', ms=0.0, marker='o', lw=1)
    ...
    ax1.axis([0,100,0,45])
    ax2.axis([0,100,0,0.0065])
    ax3.axis([0,100,0,1])

    fig.suptitle('q=(0,0,1/4)',y=0.93)

    plt.savefig(f'phonon_single_point.png',dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    ...
    plot_single_point(hdf5_file='phonon_angmom_calcs.hdf5')
