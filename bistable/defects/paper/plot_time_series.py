...
def get_const_j_data(j,y,z):
    ...
    filename =  f'results_v_y_{y:.3f}_z_{z:.3f}.h5'
    _n, _x, _v, _j, _, _ = get_j_data(filename)

    _ind = np.nanargmin(np.abs(j-_j))
    v = _v[_ind]
...
def get_time_series(y,z):
    ...
    mode = 'const_v'
    v_set = np.linspace(0,0.5,num_step // 2)
...
y = 0.05
z = 0.5
time, v, j = get_time_series(y,z)
ax[0].plot(time,v,c=blue,lw=2,ls='-')
tax[0].plot(time,j,c=orange,lw=2,ls='-')
ax[0].annotate(f'y={y:.3f}',xy=(0.5,0.91),xycoords='axes fraction',c='k',fontsize=12)
ax[0].annotate(f'z={z:.3f}',xy=(0.5,0.835),xycoords='axes fraction',c='k',fontsize=12)
ax[0].annotate(f'(a)',xy=(0.05,0.91),xycoords='axes fraction',c='k',fontsize=12)

y = 0.1
z = 1.0
time, v, j = get_time_series(y,z)
ax[1].plot(time,v,c=blue,lw=2,ls='-')
tax[1].plot(time,j,c=orange,lw=2,ls='-')
...
z = 0.5
time, v, j = get_time_series(y,z)
ax[2].plot(time,v,c=blue,lw=2,ls='-')
tax[2].plot(time,j,c=orange,lw=2,ls='-')
...
ax[0].set_ylabel('v',color=blue,fontsize=16)
tax[2].set_ylabel('j',color=orange,fontsize=16)
...
for ii in range(3):
    ...
    ax[ii].axis([0,1.05,0.0,0.5])
    tax[ii].axis([0,1.05,0,0.01])

    ax[ii].set_xlabel('time',fontsize=16)

    ax[ii].tick_params(axis='y', colors=blue)
    tax[ii].tick_params(axis='y', colors=orange)

    ax[ii].axhline(0,lw=1,ls=(0,(1,1)),c='k')
    tax[ii].axhline(0,lw=1,ls=(0,(1,1)),c='k')

    # ax[ii].yaxis.set_label_position("left")
    # ax[ii].yaxis.tick_left()

    tax[ii].yaxis.set_label_position("right")
    tax[ii].yaxis.tick_right()
