...
class c_bistable_defects:
    ...
    def __init__(self,v=0.2,y=0.1,x_lo=None,x_hi=5,num_x=50001):
    ...
    def solve(self,n_lo_guess=0.0,n_hi_guess=1.0,max_iter=1000,n_tol=1e-9,alpha=0.4,
              degeneracy_tol=1e-3,count=None):
        ...
        return multistable, n_lo, x_lo, n_hi, x_hi, n_diff, x_diff
...
def run_v_sweep(y=0.025):
    ...
    multistable = np.zeros(nv,dtype=bool)
    ...
    for ii, vv in enumerate(v):
        ...
        bistable = c_bistable_defects(vv,y)
        multistable[ii], n_lo[ii], x_lo[ii], n_hi[ii], x_hi[ii], \
            n_diff[ii], x_diff[ii] = bistable.solve(count=count)

        count += 1

    # write results to hdf5 file
    with h5py.File('results_v_sweep.h5','w') as db:
