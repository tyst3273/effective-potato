...
def damped_quasinewton(func,n_in,residual,alpha=0.2):

    """
    generate the next guess based on damped quasinewton algorithm (same as preconditioning with
    P calculated from finite differences)
    """

    _deriv = scipy.differentiate.derivative(func,x=n_in,initial_step=1e-6,maxiter=100)

    if not _deriv.success:
        print('')
        print(_deriv)
        exit('fuck 2.0')
    ...
    return n_in - alpha * residual / _deriv
...
class c_bistable_defects:
    ...
    def _solve_constant_j_newton(self,n_in):
    ...
    def solve_constant_j(self,j,n_guess=0.1,max_iter=200,n_tol=1e-9,alpha=0.4,method='anderson'):
        ...
        if self.method == 'newton':
            n_0, x_0 = self._solve_constant_j_newton(n_guess)
        else:
            n_0, x_0 = self._solve_constant_j_self_consistent(n_guess)
    ...
    def _solve_constant_j_self_consistent(self,n_in):
        ...
        for iter in range(_max_iter):
            ...
            elif self.method == 'damped_quasinewton':
                _func = lambda x : self._calc_n_constant_j(x_0,x)-x
                n_in = damped_quasinewton(_func,n_in,residual)
            else:
                print(f'\n*** ERROR ***\nmethod {self.method} is unknown')
                exit()
...
def run_j_guess_array(y=0.1,z=0.1):
    ...
    bistable = c_bistable_defects(y=y,z=z,x_hi=1,num_x=1001)

    num_j = 1001
    j = np.linspace(0.0,0.1,num_j)
    # j = np.logspace(-9,-6,num=num_j)

    num_guesses = 11
    # n_guess = np.linspace(0,1.0,num_guesses)
    # n_guess[0] = 0.001
    # n_guess[-1] = 0.999
    n_guess = np.logspace(-6,0,num_guesses)
    n_guess[-1] = 0.999
    ...
    for ii, jj in enumerate(j):
        for kk, nn in enumerate(n_guess):
            ...
            print(f'\nnow doing count: {count}')
            n_solutions[ii,kk], x_solutions[ii,kk] = bistable.solve_constant_j(jj,n_guess=nn,
                                                                    n_tol=1e-9,
                                                                    method='newton')
            count += 1
...
def run_j(y=0.1,z=0.1):
    ...
    for ii, jj in enumerate(j):
        ...
        bistable = c_bistable_defects(y=y,z=z,x_hi=5.0,num_x=10001)

        # n_lo[ii], x_lo[ii] = bistable.solve_constant_j(jj,n_guess=0.001,n_tol=1e-9,
        #                                                method='simple')
        # n_hi[ii], x_hi[ii] = bistable.solve_constant_j(jj,n_guess=0.5,n_tol=1e-9,
        #                                                method='simple')

        n_lo[ii], x_lo[ii] = bistable.solve_constant_j(jj,n_guess=0.1,n_tol=1e-6,
                                                       method='damped_quasinewton')
        n_hi[ii], x_hi[ii] = bistable.solve_constant_j(jj,n_guess=0.5,n_tol=1e-6,
                                                       method='damped_quasinewton')

        # n_lo[ii], x_lo[ii] = bistable.solve_constant_j(jj,n_guess=0.001,n_tol=1e-9,
        #                                                method='newton')
        # n_hi[ii], x_hi[ii] = bistable.solve_constant_j(jj,n_guess=0.1,n_tol=1e-9,
        #                                                method='newton')
