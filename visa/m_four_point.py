...
def get_control():
    ...
    ctrl = c_four_point_control()
    # ps.connect_resource(resource='TCPIP0::192.168.0.2::5025::SOCKET')
    ctrl.connect_resources(ps='ASRL4::INSTR',vm='GPIB0::16::INSTR')
    ctrl.print_ids()
    return ctrl
...
# ----------------------------------------------------------------------------------------------
    ...
    def connect_resources(self,ps='ASRL4::INSTR',vm='GPIB0::16::INSTR'):
        ...
        try:
            self.ps = self.resoure_manager.open_resource(ps)
            self.ps.timeout = 5000
            self.ps.read_termination = '\n'
            self.ps.write_termination = '\n'
            _id = self.ps.query('*IDN?')

        except Exception as ex:
            print(f'\nfailed to connect/get id for {ps}\n  '+str(ex)+'\n')

        # clear errors
        while True:

            err = self.get_ps_error()
            if err.split(',')[0] == '0':
                break   

        # connect voltmeter supply
        try:
            self.vm = self.resoure_manager.open_resource(vm)
            self.vm.timeout = 5000
            self.vm.read_termination = '\n'
            self.vm.write_termination = '\n'
            _id = self.vm.query('*IDN?')

        except Exception as ex:
            print(f'\nfailed to connect/get id for {vm}\n  '+str(ex)+'\n')
            self.vm = None

        # clear errors
        while True:

            err = self.get_vm_error()
            if err.split(',')[0] == '0':
                break   
    
    # ----------------------------------------------------------------------------------------------

    def get_ps_error(self):

        """
        get error
        """

        return self.ps.query('SYST:ERR?')
    
    # ----------------------------------------------------------------------------------------------

    def get_vm_error(self):

        """
        get error
        """

        return self.vm.query('SYST:ERR?')
    ...
    def print_ids(self):
        ...
        print('\nps:',self.ps_key)
        print(self.ps.query('*IDN?'))
        # print('version:',self.ps.query("SYST:VERS?"))

        print('\nvm:',self.vm_key)
        print(self.vm.query('*IDN?'))
    ...
    def remote(self):
        ...
        # if ps_control.ps.query('SYST:LOCK:OWNER?').strip() == 'NONE':
        #     self.ps.write('SYST:LOCK ON')
        self.ps.write('SYST:LOCK ON')
    ...
    def local(self):
        ...
        # if ps_control.ps.query('SYST:LOCK:OWNER?').strip() == 'REMOTE':
        #     self.ps.write('SYST:LOCK OFF')
        self.ps.write('SYST:LOCK OFF')
    ...
    def set_voltage(self,voltage):
        ...
        return self.ps.write(f'VOLT {voltage}')
    ...
    def set_current(self,current):
        ...
        return self.ps.write(f'CURR {current}')
    ...
    def measure_voltage(self):
        ...
        """
        measure output voltage (volts) from powersupply
        """

        return float(self.ps.query('MEAS:VOLT?').strip().strip('V'))
    
    # ----------------------------------------------------------------------------------------------

    def measure_probe_voltage(self):

        """
        measure input voltage (volts) from voltmeter
        """
        
        if self.vm is None:
            return 0.0
        
        return float(self.vm.query('MEAS:VOLT?').split(',')[0].strip('NVDC')) 
    ...
    def measure_current(self):
        ...
        return float(self.ps.query('MEAS:CURR?').strip().strip('A'))
    ...
    def _logger(self, filename, interval, fmt=' 012.6e'):
        ...
        with open(filename,'w') as f:
            ...
            f.write('   TIME[S]       V-OUT[V]      I-OUT[A]      V-SET[V]      I-SET[A]      V-VM[V]\n')
            t0 = None
            ...
            while self.logging:
                ...
                try:
                    ...
                    _V4 = self.measure_probe_voltage()
                    ...
                    _line = f'{_t:{fmt}} {_V:{fmt}} {_I:{fmt}} {_V0:{fmt}} {_I0:{fmt}} {_V4:{fmt}}\n'
                    f.write(_line)
                    f.flush()

                except Exception as ex:
                    print('logging error:',ex)
                    print(self.get_ps_error())
                    print(self.get_vm_error())
...
if __name__ == '__main__':
    ...
    ctrl.start_sequencing(10,0.001)
    # time.sleep(1)
