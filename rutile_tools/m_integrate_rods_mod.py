...
class c_integrate_rods:
    ...
    def plot_volume(self,Q_bin_center,u_binning,v_binning,w_binning,shell_thickness=None,
        ...
        # plot a 'shell' centered on Q_plot_center
        if shell_thickness is not None:
            _Q_len = np.linalg.norm(self.Q_plot_center)
            _d = np.sqrt(x**2+y**2+z**2)
            _shell = (_d < _Q_len + shell_thickness)*(_d > _Q_len - shell_thickness)
            _shell = _shell.astype(float)
            contours = [1.0]
            mlab.contour3d(x,y,z,_shell,contours=contours,color=(0,1,0),
                transparent=True,opacity=0.1,figure=fig)
        
        # plot the binning projeciton vectors
        if plot_uvw:
    ...
    def save_volume_plot(self,Q_bin_center,u_binning,v_binning,w_binning,shell_thickness=None,
            Q_plot_center=None,scale=2e4,stride=1,plot_uvw=False,uvw=None,fig_prefix=None):
        """
        read in volume to be intergrated and plot the binning region
        """
...
def get_uvw(which='k1'):
    ...
    if which == 'k1':
        ...
        u = [ 0.205885499, 0.205885499, 0.956672526]
        v = [ 0.000000000, 0.393930001,-0.084777678]
        w = [-0.394316504, 0.017454494, 0.081104475]

    elif which == 'z2':
        u = [-0.205885499, 0.205885499, 0.956672526]
        v = [-0.393930001, 0.000000000,-0.084777678]
        w = [-0.017454494,-0.394316504, 0.081104475]
        ...
        print('unknown binning. ask for k1, k2, k3, k4, h1, h2, h3, h4, z1, z2, z3, or z4')
