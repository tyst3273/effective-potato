...
class c_MDE_tools:
    ...
    def bin_MDE_chunks(self,H_bins,K_bins,L_bins,E_bins,num_Q_mesh=[1,1,1],
        ...
        print('\nH_centers:\n',np.round((self.H_edges[1:]+self.H_edges[:-1])/2,3))
        print('K_centers:\n',np.round((self.K_edges[1:]+self.K_edges[:-1])/2,3))
        print('L_centers:\n',np.round((self.L_edges[1:]+self.L_edges[:-1])/2,3))
        print('E_centers:\n',np.round((self.E_edges[1:]+self.E_edges[:-1])/2,3))
    ...
    def _create_datasets(self,db):
        ...
        db.create_dataset('u',data=self.u,dtype=np.float32)
        db.create_dataset('v',data=self.v,dtype=np.float32)
        db.create_dataset('w',data=self.w,dtype=np.float32)
        db.create_dataset('lattice_vectors',data=self.lattice_vectors,dtype=np.float32)
        db.create_dataset('recip_lattice_vectors',data=self.recip_lattice_vectors,dtype=np.float32)
        db.create_dataset('a',data=self.a,dtype=np.float32)
        db.create_dataset('b',data=self.b,dtype=np.float32)
        db.create_dataset('c',data=self.c,dtype=np.float32)
        db.create_dataset('alpha',data=self.alpha,dtype=np.float32)
        db.create_dataset('beta',data=self.beta,dtype=np.float32)
        db.create_dataset('gamma',data=self.gamma,dtype=np.float32)
    ...
    def save_to_hdf5(self,output_file_name):
        ...
        with h5py.File(output_file_name,'w') as db:
            ...
            for ii, name in enumerate(self.dim_names):
                ...
                db.create_dataset(f'Dim_{ii}',data=self.dims[ii],dtype=np.float32)

            db.create_dataset('signal',data=self.signal,dtype=np.float64)
            db.create_dataset('error',data=self.err,dtype=np.float64)
    ...
    def bin_MDE(self,H_bins=None,K_bins=None,L_bins=None,E_bins=None,
                        u=[1,0,0],v=[0,1,0],w=None,**MDNorm_kw_args):
        """
